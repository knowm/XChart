...
public class AxisTickLabels<ST extends AxesChartStyler, S extends AxesChartSeries> implements ChartPart {
  ...
  AxisTickLabels(Chart<ST, S> chart, Direction direction, Axis yAxis, boolean paintJustXAxisTickLabels){
  ...
  private void paintYAxis(Graphics2D g, ST styler) {
  ...
  private void paintXAxis(Graphics2D g, ST styler) {
    ...
    int quantity = chart.getXAxis().getAxisTickCalculator().getTickLabels().size();
    for (int i = 0; i < quantity; i++) {
      if(paintJustXAxisTickLabels && i>0 && i<quantity-1) continue;
      String tickLabel = chart.getXAxis().getAxisTickCalculator().getTickLabels().get(i);
      // System.out.println("tickLabel: " + tickLabel);
      double tickLocation = chart.getXAxis().getAxisTickCalculator().getTickLocations().get(i);
      double shiftedTickLocation = xOffset + tickLocation;
