...
public class AxisTickLabels<ST extends AxesChartStyler, S extends AxesChartSeries>
  ...
  AxisTickLabels(Chart<ST, S> chart, Direction direction, Axis yAxis) {
    ...
    addChartPart(new AxisTickLabelsXAxisPart());
    addChartPart(new AxisTickLabelsYAxisPart());
  }

  class AxisTickLabelsXAxisPart implements ChartPart{
    @Override
    ...
    public void paint(Graphics2D g) {
      ...
      if (!(direction == Axis.Direction.X && styler.isXAxisTicksVisible()))return;
  ...
  class AxisTickLabelsYAxisPart implements ChartPart {
    @Override
    public Rectangle2D getBounds() {
      return bounds;
    }
    ...
    public void paint(Graphics2D g)  {
      ST styler = chart.getStyler();
      g.setFont(styler.getAxisTickLabelsFont());
      if (!(direction == Axis.Direction.Y && styler.isXAxisTicksVisible()))return;
